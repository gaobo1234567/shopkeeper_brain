...
def create_app() -> FastAPI:
    ...
    # 2. 跨域配置（当前项目不会出现）---浏览器会出现
    # 只要协议 + 域名/IP + 端口这三个东西有一个不同，就算跨域。
    app.add_middleware(
        CORSMiddleware,
    ...
    # 3. 将静态资源的目录挂载到app实例上
    # 前端发请求时，只要目录里面带有front，就会使用front_page_dir目录下的资源。
...
# 运行这个文件之后，访问http://localhost:8000/front/import.html就是导入文档的界面。