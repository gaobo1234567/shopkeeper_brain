...
请根据文档标题和内容切片，识别这份文档主要描述的核心对象，并返回一个最适合用于后续检索和区分不同文档的规范名称。

【提取要求】
1. 优先提取文档真实描述的对象名称，不要直接照搬不具有实际语义的文件名。
2. 名称应尽量具体，能够区分不同对象。
3. 如果文档中明确存在品牌、型号、版本、系列等具有区分度的信息，可以自然地包含在名称中。
4. 不要求文档必须具有品牌、型号等字段；对于软件、服务、方案、知识主题等其他类型文档，直接提取其最明确的核心对象名称。
5. 不要根据常识补充文档中未明确出现的信息。
6. 只返回最终名称，不要解释，不要输出 JSON。
7. 如果根据提供的信息无法确定核心对象，请返回 UNKNOWN。
