...
# enable_dynamic_field=True:表示的是schema中没有定义的约束，但是插入数据的时候，有不在schema中没有定义的字段。允许插入进去。
# 切记：不是我定义的，你插入数据的时候可以不传。所以这里不传part，因为有的chunk并没有参与切分，没有这个字段。
# 这里定义的是标量字段中一定会出现的
...
class _MilvusSchemaBuilder:
    ...
    def build(client: MilvusClient, dim: int) -> CollectionSchema:
        ...
        # 2. 构建主键字段约束
        # 插入时不需要传入 chunk_id，Milvus 会自动生成。开启 AutoID 后由 Milvus 自动分配。
        schema.add_field(
            field_name="chunk_id",
...
class ImportMilvusNode(BaseNode):
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        # 5.确保集合存在（判断集合是否有，没有就创建新的schema和index）
    ...
    def _validate_get_inputs(self, state: ImportGraphState):
        ...
        for chunk in chunks:
            ...
            if chunk.get('dense_vector') and chunk.get('sparse_vector'):
                #经过筛选后合格的chunk加入到validated_chunks中
