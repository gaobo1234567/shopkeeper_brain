...
# Entity标签节点的创建
# 这里使用两个{}的原因是外层使用时先调用了一个format函数，会使用一个{}
# 这个语句存在一个问题，一个文档中的一个实体在多个chunk都出现时，只能关联到一个。
...
class ProcessingStats:
    ...
    total_chunks: int = 0 # 总共有多少块
    processed_chunks: int = 0 #
    failed_chunks: int = 0 # 失败的块
    total_entities: int = 0 # 总共提取出来多少实体
    total_relations: int = 0 # 总共提取出来多少关系
...
class KnowLedgeGraphNode(BaseNode):
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        # 3. 获取milvus客户端和neo4j驱动
        milvus_client = get_milvus_client()
        neo4j_driver = get_neo4j_driver()

        # 4. 删除已经存在的数据：幂等性保证
        # 4.1 删除milvus集合中该item_name所在文档生成的实体名字的记录（delete:item_name）
        # 4.2 删除neo4j的整个库下的所有节点以及关系）
    ...
    def _process_single_chunk(self, chunk_id: str,
        ...
        # 3. 写入
        # 3.1 将清洗后的实体名字（可能是多个）存储到milvus（用于后续检索时候对齐）
        # 这里存完之后，向量数据库中存了三样东西：
        # （1）每一个文档对应的一个item_name
        # （2）每一个chunk中的稀疏向量和稠密向量
        # （3）每一个chunk中提取出来的实体名字
    ...
    def _clean_entities(self, entities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        unique_seen = set() # 里面保存已经存在的（实体名+标签）组合，用于去重
    ...
    def _clean_relations(self, cleaned_unique_entity_names: Set[str], relations: List[Dict[str, Any]]) -> List[
        ...
        for relation in relations:
            ...
            # 1.4 判断头尾实体名是否超过阈值
            # （因为cleaned_unique_entity_names中存储的是经过截断处理的名字，所以在判断是否在集合内之前，也需要进行截断）
