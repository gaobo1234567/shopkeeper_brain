...
class ItemNameRecognitionNode(BaseNode):
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        # 4. 嵌入商品名(嵌入模型：OpenAI:OpenAIEmbeddings dashscope:text_embedding_v1(2/3/4)--->只能生成稠密向量：主要作用（语义相似性）、
        # 想要再多生成稀疏向量 主要作用（关键词相似）【bgem3】)
    ...
    def _recognition_item_name_by_llm(self, file_title: str, item_name_context: str) -> str:
        ...
        try:
            ...
            # 3.1 获取模型的输出内容，比llm_response.content更安全
    ...
    def _save_to_milvus(self, file_title, item_name, dense_vector, sparse_vector, config):
        ...
        try:
            ...
            # 2.5 插入数据到Milvus:
            # result结构
            # {"insert_count":10,插入数据的数量
            # "ids":[10001,10002,10003] 插入数据的id}
...
if __name__ == '__main__':
    ...
    result = item_name_recognition_node(state)
    # 5. 输出
