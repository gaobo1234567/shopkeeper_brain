...
class PdfToMdNode(BaseNode):
    ...
    def _validate_state_inputs_path(self, state: ImportGraphState) -> Tuple[Path, Path]:
        ...
        if not import_file_path_opj.exists():
            raise FileProcessingError("解析的文件路径不真实存在", self.name)
    ...
    def _execute_mineru(self, import_file_path: Path, file_dir_path: Path):
        ...
        proc = subprocess.Popen(args=cmd,
                                ...
                                stderr=subprocess.STDOUT,#错误日志输出
    ...
    def _get_md_paths(self, import_file_path: Path, file_dir_path: Path):
        # file_dir_path:D:\pycharmprojects\shopkeeper_brain\knowledge\processor\import_process\output_temp_dir
...
if __name__ == '__main__':
    ...
    pdf_to_md_node_init_state = {
        ...
        "file_dir": r"D:\pycharmprojects\shopkeeper_brain\knowledge\processor\import_process\output_temp_dir"
    }
