...
class ItemNameAligner():
    ...
    def _match_vector(self, item_names: List[str]) -> List[Dict[str, Any]]:
        ...
        Returns:
             ...
             Dict[str,Any]:字典中有两个key，分别是：
             {
                "extracted_name":"LLM提取出来的商品名字"，
                "matches":[{"item_name":"向量数据库的商品名","score":"结果分数值"}]
            }
    ...
    def _item_name_score_align(self, search_results: List[Dict[str, Any]]) -> Tuple[List[str], List[str]]:
        ...
        Returns:
            ...
            几种返回值情况分析：（根据decide函数逻辑）
            （1）只要confirmed有，options有或者没有不管，将confirmed中的商品名 传给下游四路检索
            （2）只有options有，confirmed没有，确认下一步，询问到底在咨询哪一款商品。
            （3）confirmed没有 options没有，直接告诉没有找到具体的商品名
            （4）confirmed有 options有，至少确定了一个商品名，没有必要让用户再次确认这个商品。和（1）一样

            注意：
            1. 如果向confirmed列表中添加某一次遍历向量数据库查询到的商品名时，发现confirmed已经有该商品名了，就不需要去重复添加了。
            2. 如果向options列表中添加某一次遍历向量数据库查询到的商品名时，发现options已经有该商品名了，就不需要去重复添加了。
            ...
            所以去重的方向是单向的
            ToDo： options部分做的不够严谨，后续增加和用户交互的部分，选择是进入confirmed还是抛弃
        """
        ...
        for item_name_search_result in search_results:
            ...
            # 询问是否能进入到confirmed中
            if high:
                # 3.1 准备找最精准的那一个（llm提取出来的和向量数据库提取出来的完全一致）
                ...
                # 场景C:多个相似
                    ...
                    # 如果没有找到精确的 & high中还有多个
                    # 有两种做法：一种是放到confirmed中，一种是放到options中。，这里选择放到options中
                    for h in high[:3]:
