...
def _build_item_entity_pairs(aligned_entities_info: List[Dict[str, Any]]) -> List[ItemEntityPair]:
    ...
    注意：之前align去重去的是entities_aligned_name，但现在遍历的是entities_aligned_elements
    ...
    for aligned_entity_info in aligned_entities_info:
        ...
        if key not in seen:
            ...
            item_entity_pairs.append({
                ...
                "entity_name": aligned_entity_name # 对齐之后的实体名
            })
    # 3. 返回
...
class _EntityAligner:
    ...
    def _align_one(self, milvus_client: MilvusClient,
        ...
        # 3. 执行混合搜索请求
        # ranker_weights中权重前后两个分别代表稀疏向量还是稠密向量取决于hybrid_search_requests请求中构建的顺序，也即是
        # create_hybrid_search_requests函数的返回值[dense_req, sparse_req]，所以前者是稠密向量后者是稀疏向量。
        ...
        for hit in hits:
            ...
            # 这里有一个细节，计算分数的时候是和实体名字进行计算的，也就是说如果不同的chunk提取出来的实体名字相同，那么分数是一样的
            # 这里只取第一个，会不会导致丢块呢？答案是不会，因为后续根据这个实体名字的关联关系去寻找chunk，他会把所有提取出来这个实体名字的chunk都找到，所以在对齐这端只需要留一个即可。
            # 但要注意一件事情，它找的时候也是按照同一个商品名去寻找，如果在另一个文档有相同商品名的话就找不到了，所以在保存的时候不同的商品名下同一个实体名这种情况是要都保存的。
        ...
        for item_name, best in best_by_item.items():
            ...
            results.append({
                "original": entity_name,# llm提取出来的名字
                "aligned": ent.get("entity_name"), #对齐到向量数据库中的名字
                "score": score,
                "item_name": item_name,
...
class _Neo4jGraphReader:
    ...
    def find_one_hop_relations(self, seed_nodes: List[EntitySeedNode]) -> List[OneHopRelation]:
        """
        职责：根据种子节点查询一跳的关系（双向），并且过滤掉 MENTIONED_IN 关系的节点。
        注意：1. 去重（不允许同一条边出现多次）只能出现一次。
             同一条边指的是类型和方向都相同：比如：A->B(类型：认识)和B->A(类型：认识)就不是同一条边。
             2.图谱中存储的节点和关系结构是什么 查询的时候一定要和存储的我结构保证一致。
             3. 邻居节点可以是你在一跳范围内指向的节点，也可以是别人在一跳范围内指向你的节点。

        Args:
            seed_nodes:（find_seed_nodes函数）所有种子节点（所有商品的种子节点）
            ...
            try:
                with self._session() as session:
                    ...
                    for seed_one_hop_relation in seed_one_hop_relations:
                        ...
                        # 场景：A节点是种子节点 邻居也是种子节点（A节点作为种子查询邻居节点的时候已经把他们的关系查找到了），
                        # 所以当在以邻居节点为种子查询的时候，就会出现重复的边。因此要过滤掉。
    ...
    def _execute_one_hop_relations(self, session, item_name: str, seed_name: str, kg_max_triples_per_seed: int) -> List[OneHopRelation]:
        """
    ...
    def collect_node_weight(self, seed_nodes: List[EntitySeedNode], one_hop_relations: List[OneHopRelation]) ->  List[Dict[str, Any]]:
        """
        由于之后在使用chunk_id去milvus中查询的时候不会返回分数值（因为不是按照向量去查询的），所以提前设置权重。
        一个chunk被多个节点同时指向时，会累加节点权重。
         职责：
    ...
    def find_nodes_chunk_id(self, weighted_nodes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for chunk_row in sorted_node_chunk_id:
            ...
            if chunk_id and item_name:
                # 此处的结构和另外两路混合检索返回的结构保持一致，方便后面节点统一处理。
                hits.append({
                    "id": None,
...
class _ChunkBackFiller:
    ...
    def back_fill(self, chunk_nodes_sorted: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # 2. 获取chunk_ids，返回一个列表，列表里的每个元素可以是 str 字符串，也可以是 int 整数。
        ...
        # 这里转化为（key为entity的字典）也是为了统一结构，方便后续rrf处理。
    ...
    def _collect_chunk_ids(self, chunk_nodes_sorted: List[Dict[str, Any]]) -> List[Union[str, int]]:
        ...
        for chunk_node in chunk_nodes_sorted:
            ...
            # 5. chunk_id转换
            # neo4j中的chunk_id是str类型，milvus中的chunk_id是int类型，所以需要提前转换。
